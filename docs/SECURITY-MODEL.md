...
LocalAuthentication and the subsequent file-based Keychain ACL read are separate application-enforced steps; the authentication context is not cryptographically bound to the Keychain item. Official Developer ID-signed builds make the executable ACL meaningful. Ad-hoc development builds do not provide the same assurance and must never be treated as production security artifacts. The CLI asks launchd to start only the installed helper, keeping one supervisor and one canonical signed identity for profile creation and reads; users must open Key Session.app once before using the CLI.
...
- Update checks never execute downloaded code or mutate the installation.
...
- Ad-hoc development signatures do not provide the executable identity assurance of official Developer ID-signed builds.
...
Release automation builds a universal binary from a version tag, applies hardened-runtime Developer ID signing, submits the app to Apple notarization, staples the ticket, publishes checksums and an SBOM, and creates a GitHub build-provenance attestation.
