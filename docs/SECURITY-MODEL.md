...
Lease and execution APIs never return secrets. Human profile management is the deliberate exception. Clicking a profile's pencil opens an editor with profile metadata but no secret. Clicking the eye invokes biometrics-only LocalAuthentication: Touch ID must succeed, and the sheet has no password fallback. The Keychain item trusts only Key Session's signed executable, so the approved daemon can read it without a second Keychain dialog. After that single system authorization, the daemon returns the profile's secret and an unguessable five-minute management capability to the native app. The secret may be hidden again, is never persisted by the app, and is cleared from view state when the editor closes. Save consumes the capability; cancel invalidates it. Remove Profile is available before reveal and asks for confirmation followed by fresh Touch ID approval. The daemon authenticates deletion separately, without reading or returning the secret, then removes the Keychain item, profile metadata, active leases, and editing sessions. Cancelled or failed authentication leaves that state unchanged.
...
SSH profiles are a separate `kind: "ssh"` in the additive schema-v2 contract. The daemon generates an Ed25519 identity and stores its base64 seed in the existing protected Keychain mechanism; only its public key is returned or persisted as metadata. Grants use the same visible Touch ID approval and verify the seed matches the configured public key. SSH profiles are rejected by secret execution and private-key reveal/update APIs. Ordinary secret setup cannot overwrite them. Rotation creates a new profile and requires public-key enrollment; deletion still requires human approval.

The daemon also serves the standard SSH agent protocol at `runtime/ssh-agent.sock` (0600 within the private 0700 runtime directory). It advertises only identities with active SSH leases and rechecks expiry/consumer lifetime under the service lock for every signature, even on an already connected socket. Sign requests are restricted to well-formed Ed25519 SSH public-key authentication payloads for an active identity, including the OpenSSH host-bound wire format. Accepting that format does not add destination constraints or agent session binding. The protocol cannot add, remove, unlock, export or retain keys independently of leases. Idle leases are also expired by a one-second sweep. The listener limits connections to 32 and each connection to 30 seconds; established SSH transports no longer need that socket. Restart loses all grants.

**Signing is deliberately same-user, not consumer-isolated.** The socket does not require a consumer capability. Any local process under that account may authenticate with an identity while at least one approved lease for it remains active. Revoking one consumer does not end another consumer's valid grant for the same identity. The Touch ID sheet and app disclose this. Consumer ownership remains enforced for CLI/MCP lease administration and secret execution.

This version does not enforce destination constraints, SSH session binding, automatic screen-lock revocation, or termination of existing sessions. SSH and server authorization policies control destinations/accounts; aliases are not a security boundary. Private/public key copies, other authorized identities and multiplexed connections can bypass an intended lease-only workflow and must be handled during migration. Recovery identities must be explicitly documented. Go memory clearing is best effort and is not a defense against process-memory or root compromise. The native app can approve a human SSH lease and revoke it from the normal consumer view; its transient grant response is not persisted.
