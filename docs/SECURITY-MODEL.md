...
The daemon also serves the standard SSH agent protocol at `runtime/ssh-agent.sock` (0600 within the private 0700 runtime directory). It advertises only identities with active SSH leases and rechecks expiry/consumer lifetime under the service lock for every signature, even on an already connected socket. Sign requests are restricted to well-formed Ed25519 SSH public-key authentication payloads for an active identity. The protocol cannot add, remove, unlock, export or retain keys independently of leases. Idle leases are also expired by a one-second sweep. The listener limits connections to 32 and each connection to 30 seconds; established SSH transports no longer need that socket. Restart loses all grants.
