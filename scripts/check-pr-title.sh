...
run_self_test() {
	for title in \
		'feat: add app visibility settings' \
		'fix(menu-bar): preserve the last visible surface' \
		'feat!: replace the daemon contract' \
		'chore(main): release key-session 0.6.0' \
	...
	for title in \
		'Add app visibility settings' \
		'feature: add app visibility settings' \
		'feat: ' \
